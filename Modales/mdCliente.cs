...
namespace CapaPresentacion.Modales
{
    public partial class mdCliente : Form
    {
        ...
        private void mdCliente_Load(object sender, EventArgs e)
        {
            ...
            {
                if (item.Estado)
                    dgvdata.Rows.Add(new object[] { item.Documento, item.NomnbreCompleto,item.Direccion,item.Telefono,item.Departamento,item.Municipio });
            }
        }
        ...
        private void dgvdata_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (iRow >= 0 && iColum >= 0)
            {
                _Cliente = new Cliente()
                {
                    ...
                    NomnbreCompleto= dgvdata.Rows[iRow].Cells["NombreCompleto"].Value.ToString(),
